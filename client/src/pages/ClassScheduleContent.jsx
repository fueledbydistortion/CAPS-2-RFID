...
import { exportScheduleToPDF, exportScheduleWeekToPDF } from "../utils/exportService";
...
const ClassScheduleContent = () => {
	...
	const handleExportPDF = () => {
		try {
			const result = exportScheduleToPDF(schedules, selectedDay, users, sections, skills);
			if (result.success) {
				setSnackbar({
					open: true,
	...
	// Handle weekly PDF export (Monday - Friday)
	const handleExportWeekPDF = () => {
		try {
			const result = exportScheduleWeekToPDF(schedules, users, sections, skills);
			if (result.success) {
				setSnackbar({ open: true, message: result.message, severity: 'success' });
			} else {
				setSnackbar({ open: true, message: result.error || 'Failed to export weekly PDF', severity: 'error' });
			}
		} catch (error) {
			console.error('Error exporting weekly PDF:', error);
			setSnackbar({ open: true, message: 'An error occurred while exporting weekly PDF', severity: 'error' });
		}
	};
