...
const AssignmentCard = ({
	...
	const submissionButtonLabel = !hasSubmission
		? "Submit Assignment"
		: isGraded
		? "Submitted"
		: "Resubmit Assignment";

	const resolveSubmissionClick = () => {
		if (!submission) {
			return;
		}
		if (typeof onViewSubmission === "function") {
				...
				<CardContent sx={{ p: 3 }}>
					...
					>
						<Box sx={{ display: "flex", alignItems: "center", flex: 1 }}>
							<Assignment
								sx={{ mr: 2, color: "hsl(152, 65%, 28%)", fontSize: "2rem" }}
							/>
							<Box sx={{ flex: 1 }}>
								<Typography
									variant="h6"
						...
						<Chip
							...
							size="small"
							icon={
								submission?.status === "graded" ? <CheckCircle /> : undefined
							}
						/>
					</Box>
					...
					<Box sx={{ mb: 3 }}>
						...
						{assignment.instructions && (
							<Box sx={{ mb: 2 }}>
								<Typography
									variant="subtitle2"
									sx={{ fontWeight: 600, mb: 1, color: "hsl(152, 65%, 28%)" }}
								>
									Instructions:
								</Typography>
								<Typography
								...
								submission.grade !== "" && (
									...
									>
										<Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
											...
											>
												Grade: {formatGradeValue(submission.grade)}
											</Typography>
										</Box>
										{submission.feedback && (
											<Typography
						...
						{!hasSubmission ? (
							...
							<Box sx={{ display: "flex", gap: 2, width: "100%" }}>
								<Button
									...
									onClick={() =>
										onViewSubmission && onViewSubmission(submission)
									}
									sx={{
										borderRadius: "12px",
								...
								<Button
									variant="contained"
									startIcon={isGraded ? <CheckCircle /> : <Upload />}
									onClick={isGraded ? undefined : handleSubmitAssignment}
									disabled={isGraded}
									sx={{
										background:
											"linear-gradient(45deg, hsl(152, 65%, 28%), hsl(145, 60%, 40%))",
