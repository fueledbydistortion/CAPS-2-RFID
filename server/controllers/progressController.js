...
const getSectionProgress = async (req, res) => {
	try {
		...
		for (const studentId of assignedStudents) {
			const studentProgressSnapshot = await db
...
const getProgressStats = async (req, res) => {
	try {
		...
		const notStartedLessons = progressRecords.filter(
			(p) => p.percentage === 0
		).length;
		...
		const averageProgress =
			totalLessons > 0
				? Math.round(
						progressRecords.reduce((sum, p) => sum + p.percentage, 0) /
							totalLessons
				  )
				: 0;
...
const getAttachmentProgress = async (req, res) => {
	try {
		...
		const progressSnapshot = await db
			.ref(`attachmentProgress/${userId}/${lessonId}`)
			.once("value");
...
const updateAttachmentProgress = async (req, res) => {
	try {
		...
		await db
			.ref(`attachmentProgress/${userId}/${lessonId}/${sanitizedAttachmentId}`)
			.update(updateData);
...
const updateLessonProgressBasedOnAttachments = async (userId, lessonId) => {
	try {
		...
		const attachmentProgressSnapshot = await db
			.ref(`attachmentProgress/${userId}/${lessonId}`)
			.once("value");
		const attachmentProgress = attachmentProgressSnapshot.val() || {};

		// Count viewed attachments (handle both sanitized and original IDs)
		const viewedAttachments = Object.values(attachmentProgress).filter(
			(att) => att.viewed
		).length;

		// Calculate progress percentage
		const progressPercentage = Math.round(
			(viewedAttachments / totalAttachments) * 100
		);
		...
		// Set createdAt if this is a new progress record
		const existingProgressSnapshot = await db
			.ref(`progress/${userId}/${lessonId}`)
			.once("value");
		...
		console.log(
			`Updated lesson progress for user ${userId}, lesson ${lessonId}: ${progressPercentage}% (${viewedAttachments}/${totalAttachments} attachments viewed)`
		);
	} catch (error) {
		console.error(
			"Error updating lesson progress based on attachments:",
			error
		);
	}
};
...
const getLessonProgressWithAttachments = async (req, res) => {
	try {
		...
		const progressSnapshot = await db
			.ref(`progress/${userId}/${lessonId}`)
			.once("value");
		const lessonProgress = progressSnapshot.exists()
		...
		const attachmentProgressSnapshot = await db
			.ref(`attachmentProgress/${userId}/${lessonId}`)
			.once("value");
		let attachmentProgress = attachmentProgressSnapshot.exists()
			? attachmentProgressSnapshot.val()
			: {};
		...
		res.status(500).json({
			success: false,
			error:
				"Failed to fetch lesson progress with attachments: " + error.message,
		});
	}
};
...
const getAllParentProgress = async (req, res) => {
	try {
		...
		if (userRole === "admin") {
			...
			console.log(
				`Found ${uniqueStudents.length} unique students across all sections`
			);
			...
			for (const studentId of uniqueStudents) {
				...
				const studentSections = allSections.filter(
					(section) =>
						section.assignedStudents &&
						section.assignedStudents.includes(studentId)
				);
				...
				parentProgress.push({
					...
					completedLessons: studentProgress.filter((p) => p.percentage === 100)
						.length,
					averageProgress:
			...
			const averageProgress =
				totalLessons > 0 ? Math.round(totalProgressSum / totalLessons) : 0;
		...
		if (userRole === "parent") {
			...
			if (sectionsSnapshot.exists()) {
				sectionsSnapshot.forEach((sectionChildSnapshot) => {
					...
					if (
						section.assignedStudents &&
						section.assignedStudents.includes(childId)
					) {
						childSections.push({
							...section,
							id: sectionChildSnapshot.key,
			...
			console.log(
				`Found ${childSections.length} sections for child ${childId}`
			);
			...
			const childProgressSnapshot = await db
				.ref(`progress/${childId}`)
				.once("value");
			const childProgress = [];
			...
			const totalProgressSum = childProgress.reduce(
				(sum, p) => sum + (p.percentage || 0),
				0
			);
			const averageProgress =
			...
			const parentProgress = [
				{
					...
					completedLessons: childProgress.filter((p) => p.percentage === 100)
						.length,
			...
			sectionsSnapshot.forEach((sectionChildSnapshot) => {
				...
				if (
					...
					(section.assignedTeachers &&
						section.assignedTeachers.includes(teacherId))
				) {
		...
		console.log(
			`Found ${teacherSections.length} sections for teacher ${teacherId}`
		);
		...
		const uniqueStudents = [...new Set(allStudents)];
		console.log(
			`Found ${uniqueStudents.length} unique students for teacher ${teacherId}`
		);
		...
		for (const studentId of uniqueStudents) {
			...
			// Get student's sections
			const studentSections = teacherSections.filter(
				(section) =>
					section.assignedStudents &&
					section.assignedStudents.includes(studentId)
			);
	...
	try {
		...
		const progressWithAssignments = progressRecords.map((progress) => {
			const assignmentSubmission = submissions.find(
				(sub) => sub.assignmentId === progress.lessonId
			);
			return {
				...progress,
		...
		res.json({
			...
			data: {
				...
				summary: {
					...
					completedAssignments: submissions.filter((s) => s.status === "graded")
						.length,
					...
					averageGrade: (() => {
						const convertGradeToNumber = (gradeValue) => {
							...
							if (typeof gradeValue === "string") {
								...
								return gradeMap[trimmed.toUpperCase()] ?? null;
							}
						...
						const numericGrades = submissions
							.map((s) => convertGradeToNumber(s.grade))
							.filter((gradeNumber) => gradeNumber !== null);
						...
						const average =
							numericGrades.reduce((sum, gradeNumber) => sum + gradeNumber, 0) /
...
const getAssignmentProgress = async (req, res) => {
	try {
		...
		// Get progress record
		const progressSnapshot = await db
			.ref(`progress/${userId}/${assignmentId}`)
			.once("value");
