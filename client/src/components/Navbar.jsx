...
function Navbar() {
  ...
  const navItems = [
    ...
    // { text: 'Download', path: '/download', icon: <DownloadIcon /> }
  ]
  ...
  const drawer = (
    <Box sx={{ width: 250, pt: 2 }}>
      <Box sx={{ textAlign: 'center', mb: 3 }}>
        ...
        >
          <img src={logo} alt="Smart Child Care Logo" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
        </Avatar>
        <Typography
          variant="h6"
          sx={{
      ...
      <List>
        {navItems.map((item) => (
          <ListItem
            ...
            sx={{
              color: isActive(item.path) ? 'hsl(152, 65%, 28%)' : 'hsl(220, 60%, 25%)',
              backgroundColor: isActive(item.path) ? 'rgba(31, 120, 80, 0.1)' : 'transparent',
              borderRadius: '12px',
              mx: 1,
              mb: 1,
              '&:hover': {
