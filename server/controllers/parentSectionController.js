...
const normalizeAttachments = (attachments, id) => {
  if (!attachments) return [];
  if (Array.isArray(attachments)) return attachments;
  if (typeof attachments === 'object') {
    return Object.values(attachments).map((attachment, index) => ({
      ...attachment,
      id: attachment.id || `${id}_attachment_${index}`,
    }));
  }
  return [];
};

// Helper function to get submission timestamp
const getSubmissionTimestamp = (submission) => {
  if (!submission) return 0;
  return submission.submittedAt || submission.createdAt || submission.timestamp || 0;
};
...
const getParentSectionContent = async (req, res) => {
  try {
    ...
    const associatedStudentIds = new Set([parentId]);
    ...
    for (const skill of sectionSkills) {
      ...
      if (lessonsSnapshot.exists()) {
        lessonsSnapshot.forEach((lessonChildSnapshot) => {
          ...
          modules.push({
            ...lesson,
            id: lessonChildSnapshot.key,
            attachments: normalizeAttachments(lesson.attachments, lessonChildSnapshot.key),
            skillName: skill.name,
            sectionName: section.name,
            sectionGrade: section.grade
          });
      ...
      if (assignmentsSnapshot.exists()) {
        assignmentsSnapshot.forEach((assignmentChildSnapshot) => {
          ...
          assignments.push({
            ...
            attachments: normalizeAttachments(assignment.attachments, assignmentChildSnapshot.key),
            skillName: skill.name,
            sectionName: section.name,
            sectionGrade: section.grade
          });
