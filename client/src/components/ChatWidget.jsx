...
const ChatWidget = () => {
  ...
  useEffect(() => {
    ...
    console.log('Listening to messages for conversation:', selectedConversation.conversationId);
  ...
  const handleOpenConversation = async (conversation) => {
    ...
    if (currentUser && conversation.conversationId) {
      console.log('Marking messages as read for conversation:', conversation.conversationId);
      await markMessagesAsReadDirect(conversation.conversationId, currentUser.uid);
    }
  };
                    ...
                    return (
                      <React.Fragment key={conversation.conversationId}>
                        ...
                        >
                          <ListItemAvatar>
                            <Avatar sx={{ 
                              bgcolor: otherParticipant?.role === 'teacher' ? 'primary.main' : 'secondary.main' 
                            }}>
                              <PersonIcon />
                            </Avatar>
                          </ListItemAvatar>
                            ...
                            primary={
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'space-between' }}>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                  <Typography variant="subtitle2" sx={{ fontWeight: conversation.unreadCount > 0 ? 700 : 400 }}>
                                    {otherParticipant?.name || 'Unknown User'}
                                  </Typography>
                                  <Chip
                                    label={otherParticipant?.role || 'User'}
                                    size="small"
                                    color={otherParticipant?.role === 'teacher' ? 'primary' : 'secondary'}
