...
export const sectionSchema = yup.object({
	name: yup
		.string()
		.required("Daycare center name is required")
		.min(2, "Name must be at least 2 characters")
		.max(50, "Name must be less than 50 characters")
		.trim(),
...
export const scheduleSchema = yup.object({
	day: yup
		...
		.oneOf(
			...
			"Please select a valid weekday (Monday to Friday)"
		),
	...
	timeOut: yup
		...
		.test(
			...
			function (value) {
				...
				if (!timeIn || !value) return true;
				return (
					new Date(`2000-01-01T${value}`) > new Date(`2000-01-01T${timeIn}`)
				);
			}
		),
