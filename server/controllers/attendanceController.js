...
const getAllAttendance = async (req, res) => {
  try {
    ...
    console.log('Attendance Query Params:', { date, startDate, endDate, sectionId });
    ...
    if (sectionId) {
      ...
      const schedulesSnapshot = await db.ref('schedules')
    ...
    console.log(`Filtered attendance: ${filteredRecords.length} records out of ${attendanceRecords.length} total`);
...
const markAttendance = async (req, res) => {
  try {
    ...
    if (timeIn && !status) {
      const statusResult = calculateAttendanceStatus(schedule.timeIn, timeIn, 15); // 15-minute grace period
      finalStatus = statusResult.status;
      statusMessage = `Auto-calculated: ${statusResult.isOnTime ? 'On time' : `${statusResult.minutesLate} minutes late`}`;
    }
    // If timeIn is provided and status is provided, use the provided status (don't override)
      ...
      const statusResult = calculateAttendanceStatus(schedule.timeIn, timeIn, 15);
      statusMessage = `Provided time: ${statusResult.isOnTime ? 'On time' : `${statusResult.minutesLate} minutes late`}`;
    }
    ...
    if (attendanceId) {
      ...
      const existingRecord = await db.ref(`attendance/${attendanceId}`).once('value');
      ...
      if (timeIn !== null && timeIn !== undefined) {
        ...
        console.log('Updating timeIn:', { original: timeIn, formatted: formatTo12Hour(timeIn) });
      }
      
      // Only update timeOut if provided, otherwise keep existing
      if (timeOut !== null && timeOut !== undefined) {
        updateData.timeOut = formatTo12Hour(timeOut);
        console.log('Updating timeOut:', { original: timeOut, formatted: formatTo12Hour(timeOut) });
      }
    ...
    const finalRecord = await db.ref(`attendance/${attendanceId}`).once('value');
    const finalData = finalRecord.val();

    // Send SMS notification (async, don't wait for it)
    sendAttendanceNotificationAsync(finalData, scheduleId, studentId, timeIn, timeOut);
    ...
    try {
      ...
      if (userSnapshot.exists()) {
        ...
        const scheduleData = await db.ref(`schedules/${scheduleId}`).once('value');
        ...
        const statusEmoji = finalStatus === 'present' ? '✓' : finalStatus === 'late' ? '⏰' : '✗';
        
        await createNotificationInternal({
          recipientId: studentId,
          recipientRole: student.role || 'parent',
          type: 'attendance',
          title: `Attendance: ${finalStatus.charAt(0).toUpperCase() + finalStatus.slice(1)}`,
    ...
    res.json({
      ...
      message: `Attendance marked as ${finalStatus.toUpperCase()}${statusMessage ? ': ' + statusMessage : ''}`
    });
...
const sendAttendanceNotificationAsync = async (attendanceData, scheduleId, studentId, timeIn, timeOut) => {
  try {
    ...
    if (smsResult.success) {
      console.log(`SMS sent successfully for ${attendanceType} - ${user.childName || user.firstName}`);
    } else {
      console.log(`SMS failed for ${attendanceType} - ${user.childName || user.firstName}:`, smsResult.error);
    }
  } catch (error) {
