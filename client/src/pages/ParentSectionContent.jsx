...
const normalizeAssignmentSubmissions = (assignments = [], parentId) =>
	(assignments || []).map((assignment) => {
		const submissionsSource = assignment.submissions;
		let submissions = [];

		if (Array.isArray(submissionsSource)) {
			submissions = submissionsSource.filter(Boolean);
		} else if (submissionsSource && typeof submissionsSource === "object") {
			submissions = Object.entries(submissionsSource).map(([id, payload]) => ({
				id,
				...(payload || {}),
			}));
		}
		...
		if (parentId) {
			submissions = submissions.filter(
				(submission) => submission?.studentId === parentId
			);
		}
		...
		const fallbackLatest =
			...
			(!parentId || assignment.latestSubmission.studentId === parentId)
				? {
						...assignment.latestSubmission,
						attachments: normalizeAttachmentList(
		...
		const latestSubmission = normalizedSubmissions[0] || fallbackLatest || null;

		return {
			...assignment,
			attachments: normalizeAttachmentList(
				assignment.attachments,
				`assignment_${assignment.id || "assignment"}`
			),
...
const formatSubmissionGrade = (gradeValue) => {
...
const getAssignmentStatus = (dueDate, latestSubmission) => {
	if (!dueDate) {
		return {
			text: "No due date",
			color: "default",
	...
	if (latestSubmission) {
		const submissionTimestamp = getSubmissionTimestamp(latestSubmission);
		if (submissionTimestamp) {
			const submissionDate = new Date(submissionTimestamp);
			if (!Number.isNaN(submissionDate.getTime())) {
				if (submissionDate <= dueDateObj) {
					return {
...
const ParentSectionContent = () => {
	...
	const loadContent = useCallback(async () => {
		...
		try {
			...
			if (result.success) {
				console.log("🔍 Parent Content Raw Data:", result.data);
				console.log("🔍 Raw Assignments:", result.data.assignments);
				console.log("🔍 Parent UID:", userProfile.uid);

				const normalizedAssignments = normalizeAssignmentSubmissions(
				...
				normalizedAssignments.forEach((assignment) => {
					...
					console.log(`   - Latest Submission:`, assignment.latestSubmission);
				});
				...
				setSelectedAssignment((previous) => {
					...
					return (
						normalizedAssignments.find(
							(assignment) => assignment.id === previous.id
						) || previous
					);
				});

				if (result.data.modules && result.data.modules.length > 0) {
					await loadModuleProgress(result.data.modules);
				}
			} else {
	...
	}, [loadModuleProgress, showSnackbar, userProfile?.uid]);
	...
	const handleAssignmentSubmissionSuccess = useCallback(async () => {
		console.log("🎉 Assignment submission success callback triggered");
		showSnackbar("Assignment submitted successfully!", "success");

		// Close the dialog temporarily to show loading state
		setAssignmentDialogOpen(false);

		console.log("🔄 Reloading content after submission...");

		// Reload the content
		await loadContent();

		console.log("✅ Content reloaded, reopening dialog in 100ms");

		// Reopen the dialog with the updated assignment after a brief delay
		// to ensure state has been updated
		setTimeout(() => {
			setAssignmentDialogOpen(true);
			console.log("✅ Dialog reopened");
		}, 100);
	}, [loadContent, showSnackbar]);
							...
							<Table>
								...
								<TableBody>
									{filteredContent.assignments.map((assignment) => {
										...
										const dueStatus = getAssignmentStatus(
											assignment.dueDate,
											latestSubmission
										);
										const submissionStatusKey = latestSubmission
											? latestSubmission.status ||
											  (latestSubmission.gradedAt
													...
													: null)
											: null;
										const submissionStatusMeta = submissionStatusKey
											? getSubmissionStatusMeta(submissionStatusKey)
											: null;
										const submissionGradeValue = latestSubmission
											...
											: null;
										const gradeDisplay =
											formatSubmissionGrade(submissionGradeValue);
										const submittedAtRaw = latestSubmission
											? latestSubmission.submittedAt ||
										...
										const gradedAtDisplay = (() => {
											if (!latestSubmission?.gradedAt) {
												return null;
											}

											const date = new Date(latestSubmission.gradedAt);
											return Number.isNaN(date.getTime())
												? null
												: date.toLocaleString();
										})();
										const updatedAtRaw =
											...
											latestSubmission.updatedAt !==
												latestSubmission?.gradedAt &&
											latestSubmission.updatedAt !== submittedAtRaw
												? latestSubmission.updatedAt
												: null;
										...
										const submissionTimingText = submittedAtDisplay
											? dueStatus.status === "submitted_late"
												? `Submitted late on ${submittedAtDisplay}`
										...
										const numericScore = (() => {
											...
											const candidates = [
												latestSubmission.pointsEarned,
												latestSubmission.score,
												latestSubmission.earnedPoints,
												latestSubmission.earnedScore,
												latestSubmission.gradeValue,
												latestSubmission.grade,
											];
										...
										return (
											...
											>
												...
												<TableCell>
													...
													>
														...
														{submissionStatusMeta && (
															<Chip
																label={`Submission: ${submissionStatusMeta.label}`}
																size="small"
												...
												<TableCell>
													...
													{formattedNumericScore && (
														<Typography
															variant="caption"
															color="text.secondary"
														>
															{assignment.points
																? `Score: ${formattedNumericScore} / ${assignment.points}`
																: `Score: ${formattedNumericScore} pts`}
														</Typography>
													)}
													{gradeDisplay && (
														<Typography
