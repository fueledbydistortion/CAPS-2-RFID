...
const uploadLessonFiles = async (req, res) => {
  try {
    ...
    const uploadResult = await uploadMultipleFiles(req.files, `lessons/${lessonId}`);
  ...
  try {
    ...
    if (attachment.path) {
      ...
      if (!deleteResult.success) {
        console.warn('Warning: Could not delete file from Firebase Storage:', deleteResult.error);
...
const uploadProfilePicture = async (req, res) => {
  try {
    console.log('Profile picture upload request received for user:', req.params.uid);
    ...
    if (!uploadResult.success) {
      console.error('Failed to upload to Firebase Storage:', uploadResult.error);
      return res.status(500).json({
        success: false,
