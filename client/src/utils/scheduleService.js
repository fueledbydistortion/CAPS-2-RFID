...
const apiRequest = async (endpoint, options = {}) => {
	try {
		...
		if (!response.ok) {
			// Return server response to caller (so structured error like conflict can be inspected)
...
export const createSchedule = async (scheduleData) => {
	try {
		...
		// Return server response (may contain success:false and conflict details)
		return response;
	} catch (error) {
		return {
			success: false,
...
// Real-time subscription simulation (polling-based) - DISABLED
...
export const subscribeToAllSchedules = (callback) => {
	// DISABLED - Polling causes infinite loops
	// Just call once and return empty unsubscribe
	console.warn(
		"⚠️ subscribeToAllSchedules is disabled - use loadSchedules instead"
	);
