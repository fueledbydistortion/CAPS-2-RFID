...
import { getUserById, getAllUsers, getUsersByRole } from '../utils/userService'
...
const DashboardContent = () => {
  ...
  const loadDashboardStats = async () => {
    ...
    try {
      ...
      const [usersResult, schedulesResult, skillsResult, attendanceResult] = await Promise.all([
        getAllUsers(),
        getAllSchedules(),
        getAllSkills(),
        getAllAttendance()
      ...
      if (usersResult.success) {
        ...
        setDashboardStats({
          ...
          totalSchedules: schedulesResult.success ? schedulesResult.data.length : 0,
          totalSkills: skillsResult.success ? skillsResult.data.length : 0,
          todayAttendance: attendanceStats,
          attendanceRate
        })
      } else {
        showSnackbar('Error loading dashboard statistics: ' + usersResult.error, 'error')
      }
    } catch (error) {
      showSnackbar('Error loading dashboard statistics: ' + error.message, 'error')
    } finally {
      setLoading(false)
    }
  }
              ...
              if (student.birthday) {
                ...
                if (birthdayParts.length === 2) {
                  ...
                  if (birthMonth === month && birthDay === day) {
                    const age = futureDate.getFullYear() - (student.birthYear || new Date().getFullYear() - 5)
                    upcomingEventsList.push({
                      id: `birthday-${student.id}-${dateStr}`,
                      type: 'birthday',
                      title: `${student.childName || student.firstName}'s Birthday`,
                      time: dateStr,
                      description: `${student.childName || student.firstName} turns ${age} today!`,
                      icon: <Cake />
                    })
                  }
                }
  ...
  const hasEventsOnDate = (date) => {
    const dateStr = new Date(new Date().getFullYear(), new Date().getMonth(), date).toISOString().split('T')[0]
    const dayName = new Date(new Date().getFullYear(), new Date().getMonth(), date).toLocaleDateString('en-US', { weekday: 'long' })
    ...
    const hasUpcomingEvent = upcomingEvents.some(event => event.time === dateStr)
    
    // Check if there are scheduled classes for this day of the week
    const hasScheduledClass = schedules.some(schedule => schedule.day === dayName)
    
    return hasAnnouncement || hasTodayEvent || hasUpcomingEvent || hasScheduledClass
  }
  ...
  const prepareWeeklyAttendanceData = () => {
    ...
    return {
      labels: weeks.map(week => new Date(week).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })),
      datasets: [
        {
          label: 'Present',
          data: presentData,
    ...
    return (
      <Box sx={{ 
        ...
        <Box sx={{ textAlign: 'center' }}>
          <CircularProgress size={80} sx={{ color: 'hsl(152, 65%, 28%)', mb: 3 }} />
          <Typography variant="h4" sx={{ color: 'hsl(152, 65%, 28%)', fontWeight: 600, mb: 2 }}>
            Loading Your Dashboard
          </Typography>
          <Typography variant="h6" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 600, color: 'text.secondary', mb: 1 }}>
    ...
    <Box>
      ...
      {userProfile?.role !== 'parent' && (
        <>
          ...
          <Paper sx={{ p: 4, mb: 4, background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', border: '2px solid rgba(31, 120, 80, 0.2)', borderRadius: '20px', boxShadow: '0 8px 32px rgba(31, 120, 80, 0.2)' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
              <Box sx={{ flex: 1 }}>
                <Typography variant="h4" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, mb: 1, background: 'linear-gradient(45deg, hsl(152, 65%, 28%), hsl(145, 60%, 40%))', backgroundClip: 'text', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                  Welcome, {userProfile?.firstName || (userProfile?.role === 'teacher' ? 'Teacher' : 'Admin')}!
                </Typography>
                <Typography variant="body2" sx={{ color: 'hsl(220, 60%, 40%)', fontSize: '0.95rem', mb: 2, fontWeight: 500 }}>
                ...
                <Typography variant="body1" sx={{ color: 'hsl(220, 60%, 25%)', fontSize: '1.1rem', lineHeight: 1.6 }}>
        ...
        <Card sx={{ 
          ...
          <CardContent sx={{ textAlign: 'center', p: 3 }}>
            <Avatar sx={{ width: 60, height: 60, mx: 'auto', mb: 2, background: 'linear-gradient(45deg, #4caf50, #8bc34a)' }}>
              <People sx={{ fontSize: '2rem' }} />
            </Avatar>
            <Typography variant="h4" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, color: '#2e7d32', mb: 1 }}>
              {loading ? '...' : dashboardStats.totalChildren}
            </Typography>
            <Typography variant="body2" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', color: '#388e3c' }}>Enrolled Children</Typography>
          </CardContent>
        </Card>
        ...
        <Card sx={{ 
          ...
          <CardContent sx={{ textAlign: 'center', p: 3 }}>
            <Avatar sx={{ width: 60, height: 60, mx: 'auto', mb: 2, background: 'linear-gradient(45deg, #ff9800, #ffc107)' }}>
              <School sx={{ fontSize: '2rem' }} />
            </Avatar>
            <Typography variant="h4" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, color: '#f57c00', mb: 1 }}>
              {loading ? '...' : dashboardStats.totalTeachers}
            </Typography>
            <Typography variant="body2" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', color: '#ef6c00' }}>Teachers</Typography>
          </CardContent>
        </Card>
        ...
        <Card sx={{ 
          ...
          <CardContent sx={{ textAlign: 'center', p: 3 }}>
            <Avatar sx={{ width: 60, height: 60, mx: 'auto', mb: 2, background: 'linear-gradient(45deg, #9c27b0, #e91e63)' }}>
              <Assessment sx={{ fontSize: '2rem' }} />
            </Avatar>
            <Typography variant="h4" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, color: '#7b1fa2', mb: 1 }}>
              {loading ? '...' : dashboardStats.totalSkills}
            </Typography>
            <Typography variant="body2" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', color: '#6a1b9a' }}>Learning Skills</Typography>
          </CardContent>
        </Card>
        ...
        <Card sx={{ 
          ...
          <CardContent sx={{ textAlign: 'center', p: 3 }}>
            <Avatar sx={{ width: 60, height: 60, mx: 'auto', mb: 2, background: 'linear-gradient(45deg, #00bcd4, #26c6da)' }}>
              <CheckCircle sx={{ fontSize: '2rem' }} />
            </Avatar>
            <Typography variant="h4" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, color: '#00838f', mb: 1 }}>
              {loading ? '...' : `${dashboardStats.attendanceRate}%`}
            </Typography>
            <Typography variant="body2" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', color: '#006064' }}>Today's Attendance Rate</Typography>
          </CardContent>
        </Card>
      </Box>
      ...
      <Paper sx={{ 
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Typography variant="h5" sx={{ fontWeight: 600, color: 'hsl(152, 65%, 28%)' }}>Today's Attendance Overview</Typography>
          </Box>
          <IconButton onClick={loadDashboardStats} disabled={loading} size="small">
            <Refresh />
          </IconButton>
        </Box>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', md: 'repeat(4, 1fr)' }, gap: 2 }}>
          {/* Present Students */}
          <Box sx={{ p: 3, background: 'rgba(76, 175, 80, 0.1)', borderRadius: '12px', textAlign: 'center' }}>
            <CheckCircle sx={{ color: '#4caf50', fontSize: '2rem', mb: 1 }} />
            <Typography variant="h4" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, color: '#2e7d32', mb: 0.5 }}>
              {dashboardStats.todayAttendance.present}
            </Typography>
            <Typography variant="body2" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', color: '#388e3c', fontWeight: 500 }}>
              Present
            </Typography>
            <Typography variant="caption" sx={{ color: '#388e3c', opacity: 0.8 }}>
              On time or within grace period
            </Typography>
          </Box>

          {/* Late Students */}
          <Box sx={{ p: 3, background: 'rgba(255, 152, 0, 0.1)', borderRadius: '12px', textAlign: 'center' }}>
            <Warning sx={{ color: '#ff9800', fontSize: '2rem', mb: 1 }} />
            <Typography variant="h4" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, color: '#f57c00', mb: 0.5 }}>
              {dashboardStats.todayAttendance.late}
            </Typography>
            <Typography variant="body2" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', color: '#ef6c00', fontWeight: 500 }}>
              Late
            </Typography>
            <Typography variant="caption" sx={{ color: '#ef6c00', opacity: 0.8 }}>
              After grace period
            </Typography>
          </Box>

          {/* Absent Students */}
          <Box sx={{ p: 3, background: 'rgba(244, 67, 54, 0.1)', borderRadius: '12px', textAlign: 'center' }}>
            <Info sx={{ color: '#f44336', fontSize: '2rem', mb: 1 }} />
            <Typography variant="h4" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, color: '#c62828', mb: 0.5 }}>
              {dashboardStats.todayAttendance.absent}
            </Typography>
            <Typography variant="body2" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', color: '#d32f2f', fontWeight: 500 }}>
              Absent
            </Typography>
          ...
          {/* Total Attendance */}
          <Box sx={{ p: 3, background: 'rgba(31, 120, 80, 0.1)', borderRadius: '12px', textAlign: 'center' }}>
            <People sx={{ color: 'hsl(152, 65%, 28%)', fontSize: '2rem', mb: 1 }} />
            <Typography variant="h4" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, color: 'hsl(152, 65%, 28%)', mb: 0.5 }}>
              {dashboardStats.todayAttendance.total}
            </Typography>
            <Typography variant="body2" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', color: 'hsl(220, 60%, 25%)', fontWeight: 500 }}>
              Total Records
            </Typography>
            <Typography variant="caption" sx={{ color: 'hsl(220, 60%, 25%)', opacity: 0.8 }}>
      ...
      {userProfile?.role === 'parent' && (
        <>
          ...
          <Paper sx={{ p: 4, mb: 4, background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', border: '2px solid rgba(31, 120, 80, 0.2)', borderRadius: '20px', boxShadow: '0 8px 32px rgba(31, 120, 80, 0.2)' }}>
            <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 2 }}>
              <Box sx={{ flex: 1 }}>
                ...
                <Typography variant="body1" sx={{ color: 'hsl(220, 60%, 25%)', fontSize: '1.1rem', lineHeight: 1.6 }}>
                  Track your child's progress and attendance. Stay updated with their learning journey at Kapitbahayan Child Development Center.
                </Typography>
              </Box>
              <IconButton 
                onClick={() => {
          ...
          <Paper sx={{ p: 3, mb: 4, background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', border: '2px solid rgba(156, 39, 176, 0.2)', borderRadius: '16px', boxShadow: '0 4px 20px rgba(156, 39, 176, 0.15)' }}>
            <Typography variant="h6" sx={{ fontWeight: 600, fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 600, color: '#7b1fa2', mb: 2 }}>
              Quick Actions
            </Typography>
          ...
          <Paper sx={{ p: 3, mb: 4, background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', border: '2px solid rgba(244, 67, 54, 0.2)', borderRadius: '16px', boxShadow: '0 4px 20px rgba(244, 67, 54, 0.15)' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
              <Warning sx={{ color: '#f44336', mr: 2, fontSize: 28 }} />
              <Typography variant="h6" sx={{ fontWeight: 600, fontFamily: 'Plus Jakarta Sans, sans-serif', color: '#f44336' }}>
                Tasks Due Today
              </Typography>
            </Box>
            {todayEvents.filter(event => event.type === 'announcement' && event.description?.toLowerCase().includes('deadline')).length > 0 ? (
              <List>
                {todayEvents.filter(event => event.type === 'announcement' && event.description?.toLowerCase().includes('deadline')).map((task, index) => (
                  <ListItem 
                    key={index}
                    sx={{ 
                      px: 2,
              ...
              <Box sx={{ textAlign: 'center', py: 3, backgroundColor: 'rgba(76, 175, 80, 0.05)', borderRadius: '8px' }}>
              ...
              {announcements.length > 0 && (
                <Paper sx={{ p: 3, background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', border: '2px solid rgba(31, 120, 80, 0.2)', borderRadius: '20px', boxShadow: '0 8px 32px rgba(31, 120, 80, 0.2)' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                    <Typography variant="h5" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 600, color: 'hsl(152, 65%, 28%)' }}>
                  ...
                  {announcements[currentAnnouncementIndex] && (
                    <Box sx={{ p: 2, backgroundColor: 'rgba(31, 120, 80, 0.05)', borderRadius: '12px', border: '1px solid rgba(31, 120, 80, 0.1)' }}>
                      <Typography variant="h6" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 600, color: 'hsl(152, 65%, 28%)', mb: 1 }}>
                        {announcements[currentAnnouncementIndex].title}
                      </Typography>
                      <Typography variant="body2" sx={{ color: 'hsl(220, 60%, 25%)', mb: 1 }}>
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Chip 
                          ...
                          size="small" 
                          sx={{ backgroundColor: 'hsl(152, 65%, 28%)', color: 'white' }}
                        />
                    ...
                    {announcements.map((_, index) => (
                      <Box
                        ...
                        sx={{
                          ...
                          height: 8,
                          borderRadius: '50%',
                          backgroundColor: index === currentAnnouncementIndex ? 'hsl(152, 65%, 28%)' : 'rgba(31, 120, 80, 0.3)',
                          margin: '0 4px',
                          transition: 'all 0.3s ease'
                        }}
                      />
              ...
              <Paper sx={{ p: 3, background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', border: '2px solid rgba(31, 120, 80, 0.2)', borderRadius: '20px', boxShadow: '0 8px 32px rgba(31, 120, 80, 0.2)' }}>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 1, mb: 2 }}>
                  {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, index) => (
                    <Typography key={index} variant="caption" sx={{ textAlign: 'center', fontWeight: 600, color: 'hsl(152, 65%, 28%)' }}>
                      {day}
                    </Typography>
                  ))}
                </Box>
                    ...
                    return (
                      <Box
                        ...
                        sx={{
                          ...
                          justifyContent: 'center',
                          borderRadius: '4px',
                          backgroundColor: isToday ? 'hsl(152, 65%, 28%)' : 'transparent',
                        ...
                        {hasEvents && (
                          <Box
                            sx={{
                              ...
                              height: 4,
                              borderRadius: '50%',
                              backgroundColor: isToday ? 'white' : 'hsl(152, 65%, 28%)',
                              mt: 0.25
                            }}
                          />
                        )}
          ...
          {parentDashboardLoading && !progressData.parentProgress.length && !attendanceData.length && (
            <Box sx={{ mb: 4 }}>
              {/* Loading Skeleton for Progress Section */}
              <Paper sx={{ p: 4, mb: 4, background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', border: '2px solid rgba(31, 120, 80, 0.2)', borderRadius: '20px', boxShadow: '0 8px 32px rgba(31, 120, 80, 0.2)' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
                  <School sx={{ color: 'hsl(152, 65%, 28%)', mr: 2 }} />
                  <Typography variant="h5" sx={{ fontWeight: 600, color: 'hsl(152, 65%, 28%)' }}>Your Child's Learning Progress</Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', py: 8 }}>
                  <Box sx={{ textAlign: 'center' }}>
                    <CircularProgress size={60} sx={{ color: 'hsl(152, 65%, 28%)', mb: 2 }} />
                    <Typography variant="h6" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 600, color: 'hsl(152, 65%, 28%)', fontWeight: 500 }}>
                      Loading your child's progress...
                    </Typography>
                    <Typography variant="body2" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', color: 'text.secondary', mt: 1 }}>
              ...
              <Paper sx={{ p: 4, mb: 4, background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', border: '2px solid rgba(31, 120, 80, 0.2)', borderRadius: '20px', boxShadow: '0 8px 32px rgba(31, 120, 80, 0.2)' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
                  ...
                  <Typography variant="h5" sx={{ fontWeight: 600, color: 'hsl(152, 65%, 28%)' }}>Your Child's Attendance</Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', py: 8 }}>
                  <Box sx={{ textAlign: 'center' }}>
                    <CircularProgress size={60} sx={{ color: 'hsl(152, 65%, 28%)', mb: 2 }} />
          ...
          {!(parentDashboardLoading && !progressData.parentProgress.length && !attendanceData.length) && (
            <Paper sx={{ p: 4, mb: 4, background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', border: '2px solid rgba(31, 120, 80, 0.2)', borderRadius: '20px', boxShadow: '0 8px 32px rgba(31, 120, 80, 0.2)' }}>
              ...
              ) : progressData && progressData.parentProgress && progressData.parentProgress.length > 0 ? (
              <Box>
                {/* Progress Summary */}
                {progressData.parentProgress.map((student, index) => (
                  <Card key={index} sx={{ mb: 3, p: 3, background: 'rgba(76, 175, 80, 0.05)', border: '2px solid rgba(76, 175, 80, 0.2)' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                      <Box>
                        <Typography variant="h6" sx={{ fontWeight: 600, fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 600, color: '#2e7d32' }}>
                          {student.studentName}
                        </Typography>
                        <Typography variant="body2" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', color: '#388e3c' }}>
                          {student.sections?.map(s => s.name).join(', ') || 'No sections assigned'}
                        </Typography>
                      </Box>
                      <Chip
              ...
              <Box sx={{ textAlign: 'center', py: 4 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  Progress records will appear here as your child completes lessons
                </Typography>
              </Box>
            )}
            </Paper>
          ...
          {!(parentDashboardLoading && !progressData.parentProgress.length && !attendanceData.length) && (
            <Paper sx={{ p: 4, mb: 4, background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', border: '2px solid rgba(31, 120, 80, 0.2)', borderRadius: '20px', boxShadow: '0 8px 32px rgba(31, 120, 80, 0.2)' }}>
              ...
              <Box>
                ...
                <Box sx={{ display: 'flex', gap: 3, mb: 3 }}>
                  ...
                  <Box sx={{ 
                    ...
                    <Typography variant="h4" sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, color: '#c62828', mb: 1 }}>
                ...
                <Typography variant="h6" sx={{ fontWeight: 600, mb: 2, fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 600, color: 'hsl(152, 65%, 28%)' }}>
                  Recent Attendance Records
                </Typography>
                <TableContainer component={Paper} sx={{ background: 'rgba(255, 255, 255, 0.8)' }}>
                  <Table>
                    <TableHead>
                      <TableRow>
                        <TableCell sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 600 }}>Date</TableCell>
                        <TableCell sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 600 }}>Time In</TableCell>
                        <TableCell sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 600 }}>Time Out</TableCell>
              ...
              <Box sx={{ textAlign: 'center', py: 4 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  Attendance records will appear here once your child starts attending
                </Typography>
              </Box>
            )}
            </Paper>
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle sx={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 600, color: 'hsl(152, 65%, 28%)' }}>
          Customize Dashboard
        </DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            Choose which sections to display on your dashboard. You can always change these settings later.
          </Typography>
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e) => setDashboardSettings(prev => ({...prev, showMiniCalendar: e.target.checked}))}
                />
              }
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e) => setDashboardSettings(prev => ({...prev, showAnnouncementsCarousel: e.target.checked}))}
                />
              }
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e) => setDashboardSettings(prev => ({...prev, showTodaySchedule: e.target.checked}))}
                />
              }
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e) => setDashboardSettings(prev => ({...prev, showUpcomingEvents: e.target.checked}))}
                />
              }
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e) => setDashboardSettings(prev => ({...prev, showProgressSnapshot: e.target.checked}))}
                />
              }
        ...
        <DialogActions>
          ...
          <Button onClick={handleSaveDashboardSettings} variant="contained" sx={{ background: 'linear-gradient(45deg, hsl(152, 65%, 28%), hsl(145, 60%, 40%))', '&:hover': { background: 'linear-gradient(45deg, hsl(152, 65%, 20%), hsl(145, 60%, 30%))' } }}>
            Save Changes
          </Button>
        </DialogActions>
      </Dialog>
