...
import { addStudentToSection, removeStudentFromSection } from '../utils/sectionService';
...
const SectionForm = ({ 
  ...
  const loadTeachers = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const teacherUsers = result.data.filter(user => user.role === 'teacher');
    ...
    try {
      ...
      if (result.success) {
        ...
        const parentUsers = allParents.sort((a, b) => {
          ...
          const aHasChild = (a.childFirstName || a.childMiddleName || a.childLastName) && 
            (a.childFirstName?.trim() || a.childMiddleName?.trim() || a.childLastName?.trim());
          const bHasChild = (b.childFirstName || b.childMiddleName || b.childLastName) && 
            (b.childFirstName?.trim() || b.childMiddleName?.trim() || b.childLastName?.trim());
  ...
  const handleAddStudent = async () => {
    if (selectedStudent && !assignedStudents.includes(selectedStudent.uid)) {
      ...
      if (sectionData) {
        try {
          const result = await addStudentToSection(sectionData.id, selectedStudent.uid);
          if (result.success) {
  ...
  const handleRemoveStudent = async (studentId) => {
    ...
    if (sectionData) {
      try {
        const result = await removeStudentFromSection(sectionData.id, studentId);
        if (result.success) {
  ...
  const getAvailableStudents = () => {
    return students.filter(student => !assignedStudents.includes(student.uid));
  };
      ...
      >
        {({ values, errors, touched, isSubmitting, setFieldValue }) => (
          <Form>
                ...
                <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                  ...
                  <Box sx={{ flex: '1 1 200px', minWidth: '200px' }}>
                    <Field name="teacherId">
                      {({ field, meta }) => (
                        <FormControl fullWidth required error={meta.touched && Boolean(meta.error)}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Typography variant="h6" sx={{ color: 'hsl(152, 65%, 28%)', fontWeight: 600, fontFamily: 'Plus Jakarta Sans, sans-serif'}}>
                Assigned Parents ({assignedStudents.length})
              </Typography>
              
              {/* Add Student Section */}
              <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', p: 2, border: '1px solid #e0e0e0', borderRadius: 1 }}>
                <Autocomplete
                  options={getAvailableStudents()}
                  getOptionLabel={(option) => {
                    const childName = [option.childFirstName, option.childMiddleName, option.childLastName]
                      .filter(Boolean).join(' ');
                ...
                <Button
                  ...
                  disabled={getAvailableStudents().length === 0 || loadingStudents}
                  size="small"
                  sx={{ 
                      ...
                      {getAssignedStudentsData().length === 0 ? (
                        ...
                        getAssignedStudentsData().map((parent) => (
                          <TableRow key={parent.uid}>
                            ...
                            <TableCell>
                              <Typography variant="body2" sx={{ fontWeight: 500, fontFamily: 'Plus Jakarta Sans, sans-serif', color: 'hsl(152, 65%, 28%)' }}>
                                {parent.childName && parent.childName.trim() !== '' 
                                  ? parent.childName 
                                  : 'No child info'
                                }
                              </Typography>
                            ...
                            <TableCell>
                              <IconButton
                                size="small"
                                color="error"
                                onClick={() => handleRemoveStudent(parent.uid)}
                              >
                                <Delete />
                              </IconButton>
                            </TableCell>
            ...
            <DialogActions>
              ...
              <Button 
                ...
                sx={{ background: 'linear-gradient(45deg, hsl(152, 65%, 28%), hsl(145, 60%, 40%))' }}
              >
                {(loading || isSubmitting) ? <CircularProgress size={24} /> : (sectionData ? 'Update' : 'Create')}
              </Button>
            </DialogActions>
          </Form>
        )}
