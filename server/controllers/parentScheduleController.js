...
const findActiveScheduleForParent = async (parentId) => {
	...
	// Daycare Centers where parent is assigned (parent represents their child)
	const relevantSections = sections.filter((section) => {
		return (
	...
	try {
		...
		if (attendanceId) {
			...
			if (existingRecordSnapshot.exists()) {
				existingAttendanceRecord = existingRecordSnapshot.val();
			}
		}
		...
		if (qrAttendanceType === "timeOut") {
			const isLateDeparture = statusResult.status === "late";
			const withinGraceDeparture =
				statusResult.status === "present" && !statusResult.isOnTime;
			const departureClassification = isLateDeparture
				? "late"
				: withinGraceDeparture
				? "within_grace"
				: "on_time";
			const departureMinutesLate = isLateDeparture
				? statusResult.minutesLate
				: withinGraceDeparture
				? statusResult.minutesLate
				: 0;

			const defaultDepartureNote = isLateDeparture
				? `Clocked out ${departureMinutesLate} minutes after schedule — marked as late time-out.`
				: withinGraceDeparture
				? `Clocked out ${departureMinutesLate} minutes after schedule but within the grace period.`
				: "Clocked out on time.";

			const existingNotesValue = existingAttendanceRecord?.notes?.trim?.()
				? existingAttendanceRecord.notes
				: null;
			const appendedNotes = existingNotesValue
				? existingNotesValue.includes(defaultDepartureNote)
					? existingNotesValue
					: `${existingNotesValue} | ${defaultDepartureNote}`
				: defaultDepartureNote;
			const combinedNotes = notes || appendedNotes;

			const derivedStatus = isLateDeparture ? "late" : "present";
			let nextStatus = existingAttendanceRecord?.status || null;

			if (derivedStatus === "late") {
				nextStatus = "late";
			} else if (!nextStatus || nextStatus === "absent") {
				nextStatus = "present";
			}

			attendanceData = {
				timeOut: currentTime,
				status: nextStatus || derivedStatus,
				timeOutStatus: isLateDeparture
					? "late_time_out"
					: withinGraceDeparture
					? "grace_time_out"
					: "on_time_time_out",
				...
				departureWithinGrace: withinGraceDeparture,
				departureNote: defaultDepartureNote,
				notes: combinedNotes,
			...
			// Preserve arrival metadata if it exists since we're not recalculating here
			if (existingAttendanceRecord) {
				const preservedArrivalFields = [
					"timeIn",
					"arrivalStatus",
					"timeInStatus",
					"arrivalMinutesLate",
				...
				for (const field of preservedArrivalFields) {
					if (
						existingAttendanceRecord[field] !== undefined &&
						attendanceData[field] === undefined
					) {
						attendanceData[field] = existingAttendanceRecord[field];
					}
				}
			}
		} else {
		...
		let responseMessage;
		if (qrAttendanceType === "timeOut") {
			responseMessage = attendanceData.departureNote
				? `TIME-OUT: ${attendanceData.departureNote}`
				: `TIME-OUT: Student ${
		...
		const responseData = {
			...
			statusMessage:
				qrAttendanceType === "timeOut"
					? attendanceData.notes || "Timed out via QR scan"
					: statusMessage,
		};
